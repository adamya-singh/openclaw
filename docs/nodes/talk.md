...
| `realtime.provider`                      | -                                           | `openai` for WebRTC, `google` for provider WebSocket, or a bridge-only provider through Gateway relay, such as `google-vertex` for [Gemini Live with Vertex AI credentials](/providers/google#realtime-voice-on-vertex-ai).                                    |
